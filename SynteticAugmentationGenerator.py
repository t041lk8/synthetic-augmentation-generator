...
class AugmentationGenerator():
    ...
    def _generate_mask(self, aa_size, bbox, x_bias, y_bias):
    ...
    def _generate_attention_area(self, img, bbox, increase_scale):
        ...
        mask, segment_area = self._generate_mask(aa_size, bbox, x_bias, y_bias)
    ...
    def __call__(self, guidance_scale: float = 10, num_inference_steps: int = 50, negative_prompt: str = None, bb_num: int = 1, increase_scale: float = 1.2):
        ann_id = 0
        img_id = 0
        for img_file in self.img_files:
            ...
            for cls in self.classes:
                ...
                for bbox in sample_bboxes:
                    ...
                    attention_area, mask, segment_area, attention_area_size = self._generate_attention_area(img, bbox, increase_scale)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--source_json', type=str, default=None, help="Path to JSON file with prompts and bboxes.")
    parser.add_argument('--final_json', type=str, default=None, help='Path to generated JSON file in COCO format.')
    parser.add_argument('--dir_images', type=str, default=None, help='Path to directory with original images.')
    parser.add_argument('--dir_dataset', type=str, default=None, help='Path to directory for augmented images.')
    
    parser.add_argument('--weights', type=str, default="stabilityai/stable-diffusion-2-inpainting", help='Path to directory with model. HuggingFace hub is supported.')
    parser.add_argument('--guidance_scale', type=float, default=10, help='A higher guidance scale value encourages the model to generate images closely linked to the text prompt at the expense of lower image quality. Guidance scale is enabled when guidance_scale > 1.')
    ...
    parser.add_argument('--negative_prompt', type=str, default=None, help='The prompt to guide what to not include in image generation.')
    parser.add_argument('--bbox_number', type=int, default=1, help='The number of bboxes generated for each class in one image.')
    parser.add_argument('--increase_scale', type=float, default=1.2, help='This parameter is responsible for the value by which the size of the bbox is multiplied to allocate the attention area. This parameter must be greater than 1.')
    ...
    Generator(guidance_scale=args.guidance_scale, num_inference_steps=args.num_inference_steps, bb_num=args.bbox_number, negative_prompt=args.negative_prompt, increase_scale=args.increase_scale)